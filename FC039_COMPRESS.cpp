...
bool isCheck(string str,string s,int k) {
    string tmp = "";
    for (int i = 1;i <= k;i++) {
        tmp += s;
    }

    return str.substr(1, n) == tmp;
}
...
void Solve() {
    ...
    for (int i = 1;i * i <= n;i++) {
        if (n % i == 0) {
            if (isCheck(str, str.substr(1, i), n / i)) maximize(ans, n / i);
            if (isCheck(str, str.substr(1, n / i), i)) maximize(ans, i);
        }
    }
    output << ans << str.substr(1, n / ans);
}
