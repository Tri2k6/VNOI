...
void Prepare() {
    for (int Mask = 0;Mask < mask(4);Mask++) {
        ...
        for (int i = 0;i < 3;i++) {
            int tmp = bit(Mask,i);
            int tmp1 = bit(Mask, i + 1);
            if (tmp && tmp1) ok = false;
        }
        if (ok) Approved_Mask.push_back(Mask);
    }
}

bool isCheck(int Mask1, int Mask2) {
    int tmp = Mask1 & Mask2;
    return !tmp;
}

ll CalcMask(int row, int Mask) {
    ll ans = 0;
    for (int i = 0;i < 4;i++) {
        if (bit(Mask, i)) {
            ans += a[row][i];
...
void Solve() {
    if (ans <= 0) {
        ...
        dp[0][0] = 0;
        for (int i = 0;i < n;i++) {
            for (auto Mask : Approved_Mask) {
                if (dp[i][Mask] < -1e15) continue;
                for (auto Next_Mask : Approved_Mask) {
                    if (isCheck(Mask, Next_Mask)) {
                        maximize(dp[i + 1][Next_Mask], dp[i][Mask] + CalcMask(i + 1, Next_Mask));
                    }
                }
            }
        }
